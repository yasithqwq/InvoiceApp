    ...
    public class CreateInvoiceTest : TestBase
    {
        ...
        public async Task ShouldCreateInvoiceClass()
        {
            InvoiceItemDto invoice = new InvoiceItemDto()
            {
                ...
                TotalAmount = Decimal.Multiply((decimal)4.2d, 1),
                InvoiceLines = new List<InvoiceLineDto> {
                            new InvoiceLineDto {Amount = Decimal.Multiply((decimal)6.2d, 1),LineAmount = Decimal.Multiply((decimal)7.2d, 1),Quantity=1+1,UnitPrice= Decimal.Multiply((decimal)2.2d, 1)},
                            new InvoiceLineDto {Amount = Decimal.Multiply((decimal)7.2d, 1),LineAmount = Decimal.Multiply((decimal)8.2d, 1),Quantity=1+1,UnitPrice= Decimal.Multiply((decimal)3.2d, 1)}
                        }
            };
